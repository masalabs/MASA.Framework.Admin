...
public class RoleDetailsPage : ComponentPageBase
{
    ...
    public async Task QueryRoleById(string? roleId)
    {
        ...
        Lodding = true;
        var result = await AuthenticationCaller.GetRoleByIdAsync(id.Value);
        if (result.Success)
        {
    ...
    public async Task<bool> UpdateRoleInfoAsync(EditContext context)
    {
        ...
        {
            Lodding = true;
            var request = new EditRoleRequest(Detail.Id, Detail.Name, Detail.Number, Detail.Describe, Detail.State);
