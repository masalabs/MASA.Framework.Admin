...
public class MenuPage : ComponentPageBase
{
    ...
    public async Task AddOrUpdateAsync(MenuItemResponse? item = null)
    {
        ...
        {
            var request = new AddMenuRequest(input.Code, input.Name, input.Description, input.Icon, input.ParentId, input.Url, input.Sort, input.Enabled,input.OnlyJump);
        ...
        {
            var request = new EditMenuRequest(input.Id, input.Name, input.Description, input.Icon, input.ParentId, input.Url, input.Sort, input.Enabled, input.OnlyJump);
