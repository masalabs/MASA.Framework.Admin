...
public class QueryHandler
{
    ...
    /// <returns></returns>
    private async Task<List<AuthorizeItemDto>> GetPermissionListAsync(
    ...
    private async Task<List<AuthorizeItemDto>> GetInheritPermissionListAsync(List<List<AuthorizeItemDto>> childrenPermissions)
    {
    ...
    private async Task<RoleDetailDto> GetDetailAsync(Guid roleId)
    {
        ...
        var list = new RoleDetailDto()
        {
            ...
            State = role.State,
            CreationTime = role.CreationTime,
            ChildrenRoles = role.RoleItems.Select(roleItem => roleItem.ChildrenRoleId).ToList(),
